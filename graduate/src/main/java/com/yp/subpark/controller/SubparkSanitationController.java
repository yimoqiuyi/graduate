...
//        List<Workhealth> workhealthList = workhealthService.selectAllWorkhealthBysId(Integer.parseInt(request.getParameter("sId")));
//        Sanitation sanitation = sanitationService.selectOneSanitationById(Integer.parseInt(request.getParameter("sId")));
//        int sanitationScore = sanitation.getSanitationScore();
//        User user = null;
//        for (Workhealth workhealth : workhealthList) {
//            user = userService.selectOneUserById(workhealth.getUserId()); //查询用户
//            user.setSanitationScore(user.getSanitationScore() + sanitationScore); //修改用户卫生得分
//            workhealthService.deleteWorkHealthByUserId(user.getUserId());  //删除用户卫生对应关系
